...
router.post('/banana-ai', upload.single('image'), async (req, res) => {
    try {
        ...
        formData.append('image', image.buffer, { filename: image.originalname, contentType: image.mimetype });
...
router.post('/what-music', upload.single('audio'), async (req, res) => {
    try {
        ...
        const formData = new FormData();
        formData.append('audio', audio.buffer, { filename: audio.originalname, contentType: audio.mimetype });
